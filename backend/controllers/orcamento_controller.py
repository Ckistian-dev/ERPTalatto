...
def atualizar_frete_orcamento(orcamento_id: int, frete_update: FreteUpdateRequest):
    ...
    try:
        ...
        if matched_carrier:
            transportadora_id = matched_carrier['id']
            transportadora_nome = matched_carrier['nome_razao']
            print(f"Correspondência encontrada: API='{search_name}' -> DB='{transportadora_nome}' (ID: {transportadora_id})")
        else:
        ...
        valores = (
            transportadora_id,
            transportadora_nome,
            decimal.Decimal(str(frete_update.final_shipping_cost)),
            frete_update.delivery_estimate_business_days,
            orcamento_id
        )
        ...
        return {"message": "Frete do orçamento atualizado com sucesso."}

    except mysql.connector.Error as err:
        raise HTTPException(status_code=500, detail=f"Erro de banco de dados: {str(err)}")
