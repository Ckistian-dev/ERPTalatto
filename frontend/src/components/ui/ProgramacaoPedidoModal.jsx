        ...
        const enrichedItens = await Promise.all(pedido.itens.map(async (item) => {
          ...
          if (item.id_produto) {
            try {
              ...
              toast.error(`Erro ao carregar detalhes do produto: ${item.id_produto}`);
  ...
  const handleConfirmSave = async () => {
    ...
    itensState.forEach(item => {
      if ((item.numero_a_retirar + item.numero_a_produzir) !== item.quantidade) {
         hasError = true;
         toast.error(`Erro no item "${item.descricao}": A soma de 'A Retirar' (${item.numero_a_retirar}) e 'A Produzir' (${item.numero_a_produzir}) deve ser igual à Quantidade Total (${item.quantidade}).`);
      }
      ...
      if (item.temEstoque && item.numero_a_retirar > 0 && totalSelecionadoLotes !== item.numero_a_retirar) {
         hasError = true;
         toast.error(`Erro no item "${item.descricao}": Você definiu retirar ${item.numero_a_retirar}, mas selecionou ${totalSelecionadoLotes} nos lotes. Por favor, ajuste a seleção de lotes.`);
      }
    });
