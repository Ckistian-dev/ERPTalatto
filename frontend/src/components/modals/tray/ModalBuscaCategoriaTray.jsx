// components/modals/tray/ModalBuscaCategoriaTray.jsx
...
export default function ModalBuscaCategoriaTray({ initialSearchTerm, onSelect, onClose }) {
    ...
    const handleSearch = async (e) => {
        ...
        setLoading(true);
        try {
            // Aponta para o novo endpoint de busca de categorias da Tray
            const { data } = await axios.get(`${API_URL}/tray/categorias/buscar?q=${searchTerm}`);
            setResults(data);
            if (data.length === 0) {
                toast.info("Nenhuma categoria encontrada para este termo.");
            }
        } catch (error) {
            toast.error("Erro ao buscar categorias na Tray.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-[70vh] flex flex-col">
                <h3 className="text-xl font-bold mb-4">Buscar Categoria na Tray</h3>
                    ...
                    {loading ? <p>Buscando...</p> : (
                        <ul className="space-y-2">
                            {results.map(cat => (
                                ...
                                >
                                    {/* A API da Tray retorna um 'path' que é ótimo para exibir a hierarquia */}
                                    <span className="font-semibold">{cat.path}</span>
                                    <span className="text-sm text-gray-500 ml-2">(ID: {cat.id})</span>
                                </li>
                            ))}
                        </ul>
                    )}
