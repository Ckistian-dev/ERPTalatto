...
export default function LayoutDashboard() {
  ...
  return (
    <div className="flex h-screen">
      ...
      <aside className={`${sidebarAberta ? 'w-64' : 'w-20'} bg-gray-800 text-gray-300 p-4 space-y-4 transition-[width] duration-300 overflow-y-auto`}>
        ...
        <nav className="space-y-2">
            ...
            {verificarPermissao(usuario, ['admin', 'vendedor', 'visitante']) && (
                <SidebarItem icone={<FaUserFriends />} titulo="Cadastros" aberto={menuAberto === 'geral'} toggle={() => toggleMenu('geral')} submenu={["Cadastro Geral", "Cadastro Produtos"]} sidebarAberta={sidebarAberta} links={["/cadastros/novo", "/produtos/novo"]} location={location} />
            )}
            {verificarPermissao(usuario, ['admin', 'vendedor', 'financeiro', 'visitante']) && (
                <SidebarItem icone={<FaBoxOpen />} titulo="Listagem" aberto={menuAberto === 'listagem'} toggle={() => toggleMenu('listagem')} submenu={["Cadastros", "Produtos"]} sidebarAberta={sidebarAberta} links={["/cadastros", "/produtos"]} location={location} exclude={['/cadastros/novo', '/produtos/novo']} />
            )}
            {verificarPermissao(usuario, ['admin', 'vendedor', 'visitante']) && (
            ...
            {verificarPermissao(usuario, ['admin', 'fiscal', 'visitante']) && (
                <SidebarItem icone={<FaFileAlt />} titulo="Notas Fiscais" aberto={menuAberto === 'notas'} toggle={() => toggleMenu('notas')} submenu={["Consultar NF-e"]} sidebarAberta={sidebarAberta} links={["/notas"]} location={location} exact />
            )}
      ...
      {/* LAYOUT RESTAURADO: Conteúdo principal com estilos originais */}
  ...
  return (
    <div>
      {isLinkSimples ? (
        <Link
          to={links[0]}
          className={`flex items-center gap-4 w-full px-4 py-2.5 rounded-md truncate overflow-hidden whitespace-nowrap transition-colors duration-200 ${classeAtivo}`}
        >
          {icone}
        ...
        <>
          <button
            onClick={toggle}
            className={`flex items-center justify-between text-left w-full px-4 py-2.5 rounded-md truncate overflow-hidden whitespace-nowrap transition-colors duration-200 ${classeAtivo}`}
          >
              ...
              {submenu.map((item, index) => {
                ...
                return (
                    <Link
                        to={links[index] || '#'}
                        key={index}
                        className={`block text-sm py-1.5 px-2 rounded-md transition-colors duration-200 ${classeSubItemAtivo}`}
                    >
                        {item}
                    </Link>
                )
