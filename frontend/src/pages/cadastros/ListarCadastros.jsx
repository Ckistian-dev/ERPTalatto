...
export default function Listacadastros() {
    ...
    const [colunaFiltro, setColunaFiltro] = useState('nome_razao');
    ...
    const [filtroRapidoColuna, setFiltroRapidoColuna] = useState('nome_razao');
    ...
    const buscarcadastros = async () => {
        ...
        try {
            ...
            const res = await axios.get(`${API_URL}/cadastros/paginado`, { params });
            setcadastros(res.data.resultados);
            setTotalPaginas(Math.ceil(res.data.total / itensPorPagina));
        } catch (error) {
            console.error('Erro ao buscar cadastros:', error);
            setMensagemErro("Erro ao buscar cadastros");
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        ...
        const ordemPadrao = [
            "id", "nome_razao", "cpf_cnpj", "tipo_pessoa",
            "tipo_cadastro", "situacao"
        ];
        setTodasColunas(ordemPadrao);
        const colunas = usuario.colunas_visiveis_cadastros?.length ? usuario.colunas_visiveis_cadastros : ordemPadrao;
    ...
    useEffect(() => {
        if (colunasVisiveis.length > 0) {
            if (!colunasVisiveis.includes(colunaFiltro)) {
                setColunaFiltro(colunasVisiveis[0]);
            }
        }
    }, [colunasVisiveis]);
    ...
    const exportarCSV = async () => {
        try {
            ...
            const linhas = todos.map(cadastro =>
                headers.map(h => {
                    const value = cadastro[h] || '';
    ...
    const editarcadastro = () => {
        if (!cadastroSelecionado) {
            toast.warn('Você deve selecionar um cadastro primeiro!');
            return;
        }
    ...
    const confirmarAcao = async () => {
        ...
        if (acaoPendente === 'editar') {
            ...
            navigate('/cadastros/editar', { state: { cadastro: cadastroSelecionado } });
        }
    };
    ...
    return (
        <div className="p-6">
            <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-gray-800">Cadastros</h1>
            <div className="w-auto overflow-auto">
                <div className="flex flex-wrap justify-between items-center mb-4 gap-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        <ButtonComPermissao type="button" onClick={editarcadastro} permissoes={["admin"]} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded flex items-center gap-2">
                            <FiEdit />Editar
                        </ButtonComPermissao>
                    </div>
                </div>
                ...
                {mostrarConfirmacao && (
                    <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50">
                        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
                            ...
                            <p className="text-gray-600 mb-6">Deseja realmente editar o cadastro <span className="font-bold text-gray-900">{cadastroSelecionado?.nome_razao}</span>?</p>
                    ...
                    <table className="w-full table-auto whitespace-nowrap">
                        ...
                        <tbody className="text-sm">
                            {loading ? (
                                ...
                                <tr><td colSpan={colunasVisiveis.length} className="text-center p-8 text-gray-500">Nenhum cadastro encontrado.</td></tr>
                            ) : (
                                cadastros.map(cadastro => (
                                    <tr key={cadastro.id} onClick={() => setcadastroSelecionado(cadastro)} className={`cursor-pointer hover:bg-teal-50 border-b ${cadastroSelecionado?.id === cadastro.id ? 'bg-teal-100' : ''}`}>
                                        {colunasVisiveis.map(coluna => <td key={coluna} className="p-3">{cadastro[coluna]}</td>)}
                                    </tr>
                                ))
                            )}
                        </tbody>
                ...
                <ModalFiltroColunas
                    ...
                    onChangeColuna={(index, novaColuna) => {
                        setFiltrosSelecionados((prev) => {
                            const atual = [...prev];
                            atual[index] = { ...atual[index], coluna: novaColuna, texto: '' };
                            return atual;
                        });
                    }}
                    API_URL={API_URL}
            ...
            {mostrarEditarTabela && (
                <ModalEditarTabela
                    ...
                    exemplos={[...cadastros].slice(-10).reverse()}
