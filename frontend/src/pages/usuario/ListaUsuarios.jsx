...
export default function ListaUsuarios() {
    ...
    useEffect(() => {
        ...
        const ordemPadrao = ["id", "nome", "email", "perfil", "ativo"];
        setTodasColunas(ordemPadrao);
        const colunasSalvas = usuarioLogado.colunas_visiveis_usuarios;
    ...
    const exportarCSV = async () => {
        ...
        try {
            const filtrosStr = filtrosSelecionados.map(f => `${f.coluna}:${f.texto}`).join(';');
            const params = {
                page: 1, limit: 10000, // Pega todos os dados para exportação
                filtros: filtrosStr || undefined,
                filtro_rapido_coluna: filtroRapidoTexto ? filtroRapidoColuna : undefined,
                filtro_rapido_texto: filtroRapidoTexto || undefined,
                ordenar_por: ordenacaoColuna,
            ...
            const res = await axios.get(`${API_URL}/api/usuarios/paginado`, { params });
            const todosUsuarios = res.data.resultados;
            ...
            const linhas = todosUsuarios.map(user =>
                headers.map(h => {
                    const valor = user[h];
    ...
    return (
        <div className="p-6">
            <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-gray-800">Gestão de Usuários</h1>
                    ...
                    <table className="w-full table-auto whitespace-nowrap">
                        ...
                        <tbody className="text-sm">
                            {loading ? (<tr><td colSpan={colunasVisiveis.length} className="text-center p-8">Carregando...</td></tr>)
                                : usuarios.length === 0 ? (<tr><td colSpan={colunasVisiveis.length} className="text-center p-8 text-gray-500">Nenhum usuário encontrado.</td></tr>)
                                    : usuarios.map(user => (
                                        <tr key={user.id} onClick={() => setUsuarioSelecionado(user)} className={`cursor-pointer hover:bg-teal-50 border-b ${usuarioSelecionado?.id === user.id ? 'bg-teal-100' : ''}`}>
