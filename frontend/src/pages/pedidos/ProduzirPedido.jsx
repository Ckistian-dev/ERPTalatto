...
export default function Listapedidos() {
    ...
    const atualizarSituacaoPedido = async (novaSituacao) => {
        if (!pedidoSelecionado) {
            exibirAviso("Você deve selecionar um pedido primeiro!");
            return;
        }
        try {
            await axios.patch(`${API_URL}/pedidos/situacao/${pedidoSelecionado.id}`, { situacao_pedido: novaSituacao });
            toast.success(`Pedido enviado para ${novaSituacao.toLowerCase()}!`);
    ...
    return (
        <div className="p-6">
            <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-gray-800">Pedidos em Produção</h1>
            <div className="w-auto overflow-auto">
                <div className="flex flex-wrap justify-between items-center mb-4 gap-2">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        ...
                        <button onClick={() => atualizarSituacaoPedido("Embalagem")} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded flex items-center gap-2"><FaCheckCircle /> Enviar para Embalagem</button>
                    </div>
                </div>
                    ...
                    <table className="w-full table-auto whitespace-nowrap">
                        ...
                        <tbody className="text-sm">
                            {loading ? (<tr><td colSpan={colunasVisiveis.length} className="text-center p-8">Carregando...</td></tr>)
                                : pedidos.length === 0 ? (<tr><td colSpan={colunasVisiveis.length} className="text-center p-8 text-gray-500">Nenhum pedido em produção.</td></tr>)
                                    : pedidos.map(pedido => (
            ...
            {mostrarModalItens && <ModalVisualizarItensProgramados pedido={pedidoSelecionado} onClose={() => setMostrarModalItens(false)} />}
        </div>
    );
}