...
export default function ListarProdutos() {
    ...
    const [colunaFiltro, setColunaFiltro] = useState('descricao')
    ...
    useEffect(() => {
        ...
        const ordemPadrao = [
            "id", "sku", "codigo_barras", "descricao", "unidade", "situacao", "peso_produto", "tipo_produto", "variacoes", "quantidades", "grupo", "subgrupo1", "subgrupo2", "subgrupo3", "subgrupo4", "subgrupo5", "url_imagem", "classificacao_fiscal", "origem", "valor_ipi", "gtin", "gtin_tributavel", "tabela_precos", "estoque", "localizacao", "tipo_embalagem", "peso_embalagem", "unidade_caixa", "largura_embalagem", "altura_embalagem", "comprimento_embalagem", "diametro_embalagem", "custo_produto", "id_fornecedor", "dias_preparacao", "marca", "garantia", "slug", "descricao_plataforma", "imagens_plataforma", "imagens_variacoes", "largura_produto", "altura_produto", "comprimento_produto", "diametro_produto", "material_produto", "fabricante", "criado_em"
        ];
    ...
    const importarCSV = async (event) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                // USO DA VARIÁVEL DE AMBIENTE AQUI
                const res = await axios.post(`${API_URL}/produtos/validar_importacao`, { registros });

                if (res.data.conflitos.length > 0) {
                    setConflitos(res.data.conflitos || []);
                    setNovosprodutos(res.data.novos || []);
                    ...
                    // USO DA VARIÁVEL DE AMBIENTE AQUI
                    await axios.post(`${API_URL}/produtos/importar_csv_confirmado`, {
        ...
        <div className="p-6">
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50">
                        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-gray-200 animate-fade-in">
                            ...
                            <p className="text-gray-600 mb-6">
                                Deseja realmente {acaoPendente === 'editar' ? 'editar' : 'excluir'} o produto{' '}
                                <span className="font-bold text-gray-900">{produtoSelecionado?.descricao}</span>?
                            </p>
                                    ...
                                    <tr>
                                        <td colSpan={colunasVisiveis.length}>
                                            <div className="flex items-center pl-[63vh] h-[63vh]">
                                                <span className="text-gray-500 text-lg">Nenhum produto encontrado.</span>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    <>
                                        {produtos.map((produto, i) => (
                                            <tr
                                                ...
                                                className={`cursor-pointer ${produtoSelecionado?.id === produto.id ? 'bg-gray-100' : ''}`}
                                            >
                                                {colunasVisiveis.map((coluna) => (
                                        ...
                                        {produtos.length < 15 &&
                                            Array.from({ length: 15 - produtos.length }).map((_, idx) => (
                        ...
                        <div className="flex justify-end gap-4 mt-4">
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    const todosParaImportar = [...novosprodutos, ...linhasConfirmadas];
            ...
            {mostrarEditarTabela && (
                <ModalEditarTabela
                    ...
                    exemplos={[...produtos].slice(-10).reverse()}
