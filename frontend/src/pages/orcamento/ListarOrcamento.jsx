...
export default function Listaorcamentos() {
    ...
    const buscarorcamentos = async () => {
        ...
        try {
            ...
            setMensagemErro("Erro ao buscar orcamentos")
        } finally {
            setLoading(false)
        }
    }
        ...
        reader.onload = async (e) => {
            try {
                ...
                if (res.data.conflitos.length > 0) {
                    ...
                    // Importação direta
                    // USO DA VARIÁVEL DE AMBIENTE AQUI
                    await axios.post(`${API_URL}/orcamentos/importar_csv_confirmado`, {
    ...
    const editarorcamento = () => {
        if (!orcamentoSelecionado) return exibirAviso('Você deve selecionar um orçamento primeiro!');
        ...
        <div className="p-6">
            <div className="w-auto overflow-auto">
                ...
                <div className="flex flex-wrap justify-between items-center mb-4 gap-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        {/* Botão editar */}
                        <ButtonComPermissao type="button" onClick={editarorcamento} permissoes={["admin"]} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded flex items-center gap-2"><FiEdit />Editar</ButtonComPermissao>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50">
                        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-gray-200 animate-fade-in">
                            ...
                            <p className="text-gray-600 mb-6">
                                Deseja realmente {acaoPendente === 'editar' ? 'editar' : 'excluir'} o orcamento{' '}
                                <span className="font-bold text-gray-900">{orcamentoSelecionado?.nome_razao}</span>?
                            </p>
                                    ...
                                    <tr>
                                        <td colSpan={colunasVisiveis.length}>
                                            <div className="flex items-center pl-[63vh] h-[63vh]">
                                                <span className="text-gray-500 text-lg">Nenhum orcamento encontrado.</span>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    <>
                                        {orcamentos.map((orcamento, i) => (
                                            <tr
                                                ...
                                                className={`cursor-pointer ${orcamentoSelecionado?.id === orcamento.id ? 'bg-gray-100' : ''}`}
                                            >
                                                {colunasVisiveis.map((coluna) => (
                                                    <td key={coluna} className="p-2 border whitespace-nowrap">
                                                        {formatarCampo(orcamento[coluna], coluna)}
                                                    </td>
                                                ))}
                                            </tr>
                                        ))}
                                        ...
                                        {orcamentos.length < 15 &&
                                            Array.from({ length: 15 - orcamentos.length }).map((_, idx) => (
                        ...
                        <div className="flex justify-end gap-4 mt-4">
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    const todosParaImportar = [...novosorcamentos, ...linhasConfirmadas];
            ...
            {mostrarEditarTabela && (
                <ModalEditarTabela
                    ...
                    exemplos={[...orcamentos].slice(-10).reverse()}
